...
def plot_exons_ply(df, max_ngenes = 25, id_col = 'gene_id', color_col = None, colormap = colormap, 
    ...
    **kargs: 
    ...
    title_dict_ply = {'family': 'Arial', 'color': get_default('title_color'), 'size': get_default('title_size')}
    ...
    fig.update_layout(plot_bgcolor=plot_background, font_color=plot_border, showlegend=legend)  
...
def _gby_plot_exons(df, fig, chrmd_df, genesmd_df, id_col, showinfo, tag_background, legend):
    ...
    # Plot the gene rows
    df.apply(_apply_gene, 
             args=(fig, strand, genename, gene_ix, exon_color, chrom, chrom_ix, n_exons, genelabel, geneinfo, exon_width, legend), 
             axis=1)
