...
def plot_transcript_plt(
    ...
    **kargs,
    ...
    """
    Create transcript structure plot from PyRanges object DataFrame
    ...
    **kargs:
    ...
    >>> plot_transcript_plt(df, max_ngenes=25, colormap='Set3')

    >>> plot_transcript_plt(df, color_col='Strand', colormap={'+': 'green', '-': 'red'})

    >>> plot_transcript_plt(df, limits = {'1': (1000, 50000), '2': None, '3': (10000, None)})
    ...
    chrmd_df = subdf.groupby("Chromosome").agg(
    ...
    if color_col == "Chromosome":
        genesmd_df = subdf.groupby(id_col).agg(
    ...
    if type(colormap) == list:
        ...
        colormap = {
            color_tags[i]: colormap[i % len(colormap)] for i in range(n_color_tags)
        }
    ...
    if file_size:
        ...
        x = 20
        y = (
            sum(chrmd_df.y_height) + 4 * len(chrmd_df)
    ...
    if max_ngenes > 25:
        plt.suptitle(
            "Warning! The plot integity might be compromised when displaying too many genes.",
            color="red",
            x=0.05,
            y=0.95,
        )
    ...
    subdf.groupby(id_col).apply(
        lambda subdf: _gby_plot_exons(
            subdf, axes, fig, chrmd_df, genesmd_df, id_col, showinfo, tag_background
        )
    )
...
def _gby_plot_exons(
    df, axes, fig, chrmd_df, genesmd_df, id_col, showinfo, tag_background
):
    ...
    for i in range(len(sorted_exons) - 1):
        ...
        def on_hover(event):
            ...
            contains_line, _ = intron_line[0].contains(
                event
